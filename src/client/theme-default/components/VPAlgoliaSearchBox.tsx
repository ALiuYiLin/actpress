...
import { inBrowser, useRouter } from 'actpress'
...
export interface VPAlgoliaSearchBoxProps {
...
export const VPAlgoliaSearchBox = defineComponent(function (
  ...
  let sidepanelLoader:
    Promise<typeof import('@docsearch/sidepanel-js')> | undefined
  ...
  async function initialize(userOptions: DefaultTheme.AlgoliaSearchOptions) {
    ...
    const options = {
      ...
      transformItems: (items) =>
        items.map((item) => ({ ...item, url: getRelativePath(item.url) })),

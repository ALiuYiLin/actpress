...
export interface VPNavScreenMenuLinkProps {
  item: DefaultTheme.NavItemWithLink
}

export function VPNavScreenMenuLink(props: VPNavScreenMenuLinkProps) {
  const { page } = useData()
  // Vue 版 inject(navInjectionKey).closeScreen；ActView 用 useNav 模块 context
  ...
  return (
    <VPLink
      class={['VPNavScreenMenuLink', isActiveLink.value ? 'active' : ''].join(
        ' '
      )}
      href={href.value}

...
export async function createMarkdownToActViewRenderFn(
  ...
  return async (
    ...
    // 预处理：<script setup>/<script lang="tsx"> 块替换为占位。
    // markdown-it 的 html_block type 7 以任意 `</(script|pre|style|textarea)>`
    ...
    const env: MarkdownEnv = {
      ...
      realPath: fileOrig,
      localeIndex,
      __avScriptBlocks: avScriptBlocks.length ? avScriptBlocks : undefined
    } as MarkdownEnv
...
export function createActViewSrc(
  ...
  // sfc 插件的 scripts 数组已包含 <script setup> 块；仅当外部构造
  // 只填 scriptSetup 字段（如单测）时补上，避免重复。
  ...
  // 所有 <script> 块内容一律提升到模块顶层（共享同一作用域）。
  // 不再区分 <script lang="tsx"> / <script setup> / 普通 <script>——
  // md 就是「一个 tsx 文件」：script 块是顶层代码，正文是页面组件的 JSX。
  ...
  // 收集模块顶层大写标识符（组件引用解析用）：无论是否 export
  ...
  // 双括号 {{ ... }} 保持字面文本：ActView 正文不做表达式求值——
  // 动态内容在 <script lang="tsx"> 里定义组件，正文用组件引用。
  ...
  // 的 Babel 只包裹大写命名的函数声明，匿名 default 组件不会被包，浏览器端
  // 挂载时渲染器只认 { __setup }，裸函数会崩（InvalidCharacterError）。
  // 若用户 script 块已 import defineComponent，则不重复添加（避免 Duplicate declaration）。
  const userImportsDefineComponent = topLevelCode.some((code) =>
    /import\s*\{[^}]*\bdefineComponent\b[^}]*\}\s*from/.test(code)
  )
  if (!userImportsDefineComponent) {
    parts.push(
      `import { defineComponent } from ${JSON.stringify(ACTVIEW_IMPORT)}`
    )
  }
  parts.push(`export default defineComponent(function () {`)
  parts.push(`  return () => (`)
...
/**
 * 合并多个 script 块的顶层代码，去重 import 语句。
 *
 * 多个 `<script>` 块可能各自写 `import { ref } from 'actview'`，提升到
 * 模块顶层后会重复声明（Babel 报 Duplicate declaration）。这里按
 * 「导入源 + 导入名」去重：同源同名只保留第一条，其余移除。
 * 非 import 的代码按原顺序拼接。
 */
...
/**
 * 移除代码里的 `export default ...`（页面组件由编译器生成，用户 options 无法转换）。
 * 支持跨行：`export default {` 开始的整段（直到 `}` 闭合）整体注释掉，
 * 避免残留 `name: "x"` / 孤立 `}` 等非法顶层片段。
 */
function stripExportDefault(code: string): string {
  ...
  while (i < lines.length) {
    ...
    if (/^\s*export\s+default\b/.test(line)) {
      // 单行 `export default expr;` / `export default {}` → 直接注释该行
      let depth = countBrackets(line)
      if (depth <= 0 && !/;\s*$/.test(line) && !/\}\s*$/.test(line)) {
        // 简单值形式（如 `export default foo`）也整行注释
        out.push(
          `// removed: user export-default (ActView 不支持 options API, 页面组件由本模块生成)`
        )
        i++
        continue
      }
 ...
 *
 * 规则：模块顶层声明（无论是否 export）的大写开头标识符都可作为组件引用——
 * `function X` / `const X` / `let X` / `var X` / `class X` / `export { X }`。
 * 忽略缩进 > 0 的声明（那些是函数体内/块内的局部变量，不在模块顶层作用域）。
 */
export function extractComponentNames(code: string): Set<string> {
  ...
  // 顶层声明：行首（允许 export 前缀，不允许缩进）的 function/const/let/var/class
  const declRe =
...
export function serializeHtmlToJsx(
  ...
  // 序列化
  const lines: string[] = []
  // 正文文本一律渲染为字面（JSX 字符串表达式）。不支持正文 {expr}/{{ expr }}
  // 求值——需要动态内容时，在 <script lang="tsx"> 里定义组件，正文用组件引用。
